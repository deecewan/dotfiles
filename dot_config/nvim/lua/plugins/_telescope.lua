return {
  ...
  config = function()
    ...
    local find_string_with_hidden = function()
      builtin.live_grep({
        additional_args = { "--hidden" },
      })
    end

    -- create a telescope mapping that, in normal mode, when `h` is pressed,
    -- adds the `--hidden` flag to the current live_grep search
    ...
    vim.keymap.set("n", "<leader>fn", find_string_with_hidden, { desc = "rg --hidden" })
